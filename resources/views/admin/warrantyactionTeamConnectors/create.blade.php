...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.warrantyactionTeamConnector.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.warrantyaction-team-connectors.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('warranty_action') ? 'is-invalid' : '' }}" name="warranty_action_id" id="warranty_action_id">
                    @foreach($warranty_actions as $id => $warranty_action)
                        <option value="{{ $id }}" {{ old('warranty_action_id') == $id ? 'selected' : '' }}>{{ $warranty_action }}</option>
                    @endforeach
                </select>
