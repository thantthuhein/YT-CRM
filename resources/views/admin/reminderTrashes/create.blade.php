...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.reminderTrash.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.reminder-trashes.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('reminder') ? 'is-invalid' : '' }}" name="reminder_id" id="reminder_id">
                    @foreach($reminders as $id => $reminder)
                        <option value="{{ $id }}" {{ old('reminder_id') == $id ? 'selected' : '' }}>{{ $reminder }}</option>
                    @endforeach
                </select>
