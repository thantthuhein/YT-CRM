...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.saleContractPdf.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.sale-contract-pdfs.update", [$saleContractPdf->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('sale_contract') ? 'is-invalid' : '' }}" name="sale_contract_id" id="sale_contract_id">
                    @foreach($sale_contracts as $id => $sale_contract)
                        <option value="{{ $id }}" {{ ($saleContractPdf->sale_contract ? $saleContractPdf->sale_contract->id : old('sale_contract_id')) == $id ? 'selected' : '' }}>{{ $sale_contract }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('iteration') ? 'is-invalid' : '' }}" type="number" name="iteration" id="iteration" value="{{ old('iteration', $saleContractPdf->iteration) }}" step="1">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('title') ? 'is-invalid' : '' }}" type="text" name="title" id="title" value="{{ old('title', $saleContractPdf->title) }}">
                @if($errors->has('title'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('uploaded_by') ? 'is-invalid' : '' }}" name="uploaded_by_id" id="uploaded_by_id">
                    @foreach($uploaded_bies as $id => $uploaded_by)
                        <option value="{{ $id }}" {{ ($saleContractPdf->uploaded_by ? $saleContractPdf->uploaded_by->id : old('uploaded_by_id')) == $id ? 'selected' : '' }}>{{ $uploaded_by }}</option>
                    @endforeach
                </select>
