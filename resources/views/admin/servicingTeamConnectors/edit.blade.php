...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.servicingTeamConnector.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.servicing-team-connectors.update", [$servicingTeamConnector->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('servicing_setup') ? 'is-invalid' : '' }}" name="servicing_setup_id" id="servicing_setup_id">
                    @foreach($servicing_setups as $id => $servicing_setup)
                        <option value="{{ $id }}" {{ ($servicingTeamConnector->servicing_setup ? $servicingTeamConnector->servicing_setup->id : old('servicing_setup_id')) == $id ? 'selected' : '' }}>{{ $servicing_setup }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('morphable') ? 'is-invalid' : '' }}" type="number" name="morphable" id="morphable" value="{{ old('morphable', $servicingTeamConnector->morphable) }}" step="1">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('morphable_type') ? 'is-invalid' : '' }}" type="text" name="morphable_type" id="morphable_type" value="{{ old('morphable_type', $servicingTeamConnector->morphable_type) }}">
