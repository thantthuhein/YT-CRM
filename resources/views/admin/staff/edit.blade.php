...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.staff.update", [$staff->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label for="phone_number">{{ trans('cruds.staff.fields.phone_number') }}</label>
                <input class="form-control {{ $errors->has('phone_number') ? 'is-invalid' : '' }}" type="text" name="phone_number" id="phone_number" value="{{ old('phone_number', $staff->phone_number) }}">
                @if($errors->has('phone_number'))
                    <span class="text-danger">{{ $errors->first('phone_number') }}</span>
                @endif
                <span class="help-block">{{ trans('cruds.staff.fields.phone_number_helper') }}</span>
            </div>
            <div class="form-group">
