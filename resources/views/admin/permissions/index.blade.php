...
@can('permission_create')
    <div style="margin-left: 30px;" class="my-3">
        <a class="btn btn-create" href="{{ route("admin.permissions.create") }}">
                    ...
                    @foreach($permissions as $key => $permission)
                        <tr data-entry-id="{{ $permission->id }}">
                            ...
                            <td>
                                <div class="d-flex">
                                    @can('permission_show')
                                        <a class="btn btn-sm btn-create mr-2" href="{{ route('admin.permissions.show', $permission->id) }}">
                                            {{ trans('global.view') }}
                                        </a>
                                    @endcan
    
                                    @can('permission_edit')
                                        <a class="btn btn-sm btn-create mr-2" href="{{ route('admin.permissions.edit', $permission->id) }}">
                                            {{ trans('global.edit') }}
                                        </a>
                                    @endcan
