...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.airconTypeConnector.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.aircon-type-connectors.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('enquiries') ? 'is-invalid' : '' }}" name="enquiries_id" id="enquiries_id">
                    @foreach($enquiries as $id => $enquiries)
                        <option value="{{ $id }}" {{ old('enquiries_id') == $id ? 'selected' : '' }}>{{ $enquiries }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('aircon_types') ? 'is-invalid' : '' }}" name="aircon_types[]" id="aircon_types" multiple>
                    @foreach($aircon_types as $id => $aircon_type)
                        <option value="{{ $id }}" {{ in_array($id, old('aircon_types', [])) ? 'selected' : '' }}>{{ $aircon_type }}</option>
                    @endforeach
                </select>
