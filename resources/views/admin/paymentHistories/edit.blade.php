    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.payment-histories.update", [$paymentHistory->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('sale_contract') ? 'is-invalid' : '' }}" name="sale_contract_id" id="sale_contract_id" required>
                    @foreach($sale_contracts as $id => $sale_contract)
                        <option value="{{ $id }}" {{ ($paymentHistory->sale_contract ? $paymentHistory->sale_contract->id : old('sale_contract_id')) == $id ? 'selected' : '' }}>{{ $sale_contract }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('payment_step_from') ? 'is-invalid' : '' }}" type="number" name="payment_step_from" id="payment_step_from" value="{{ old('payment_step_from', $paymentHistory->payment_step_from) }}" step="1">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('payment_step_to') ? 'is-invalid' : '' }}" type="number" name="payment_step_to" id="payment_step_to" value="{{ old('payment_step_to', $paymentHistory->payment_step_to) }}" step="1">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('created_by') ? 'is-invalid' : '' }}" name="created_by_id" id="created_by_id">
                    @foreach($created_bies as $id => $created_by)
                        <option value="{{ $id }}" {{ ($paymentHistory->created_by ? $paymentHistory->created_by->id : old('created_by_id')) == $id ? 'selected' : '' }}>{{ $created_by }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('updated_by') ? 'is-invalid' : '' }}" name="updated_by_id" id="updated_by_id">
                    @foreach($updated_bies as $id => $updated_by)
                        <option value="{{ $id }}" {{ ($paymentHistory->updated_by ? $paymentHistory->updated_by->id : old('updated_by_id')) == $id ? 'selected' : '' }}>{{ $updated_by }}</option>
                    @endforeach
                </select>
