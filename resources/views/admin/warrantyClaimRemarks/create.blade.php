...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.warrantyClaimRemark.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.warranty-claim-remarks.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('warranty_claim') ? 'is-invalid' : '' }}" name="warranty_claim_id" id="warranty_claim_id">
                    @foreach($warranty_claims as $id => $warranty_claim)
                        <option value="{{ $id }}" {{ old('warranty_claim_id') == $id ? 'selected' : '' }}>{{ $warranty_claim }}</option>
                    @endforeach
                </select>
