...
<div class="card content-card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.servicingContract.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.servicing-contracts.update", [$servicingContract->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('project') ? 'is-invalid' : '' }}" name="project_id" id="project_id">
                    @foreach($projects as $id => $project)
                        <option value="{{ $id }}" {{ ($servicingContract->project ? $servicingContract->project->id : old('project_id')) == $id ? 'selected' : '' }}>{{ $project }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                @foreach(App\ServicingContract::INTERVAL_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('interval') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="interval_{{ $key }}" name="interval" value="{{ $key }}" {{ old('interval', $servicingContract->interval) === (string) $key ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('contract_start_date') ? 'is-invalid' : '' }}" type="text" name="contract_start_date" id="contract_start_date" value="{{ old('contract_start_date', $servicingContract->contract_start_date) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('contract_end_date') ? 'is-invalid' : '' }}" type="text" name="contract_end_date" id="contract_end_date" value="{{ old('contract_end_date', $servicingContract->contract_end_date) }}">
            ...
            <div class="form-group">
                ...
                <textarea class="form-control ckeditor {{ $errors->has('remark') ? 'is-invalid' : '' }}" name="remark" id="remark">{!! old('remark', $servicingContract->remark) !!}</textarea>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('created_by') ? 'is-invalid' : '' }}" name="created_by_id" id="created_by_id">
                    @foreach($created_bies as $id => $created_by)
                        <option value="{{ $id }}" {{ ($servicingContract->created_by ? $servicingContract->created_by->id : old('created_by_id')) == $id ? 'selected' : '' }}>{{ $created_by }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('updated_by') ? 'is-invalid' : '' }}" name="updated_by_id" id="updated_by_id">
                    @foreach($updated_bies as $id => $updated_by)
                        <option value="{{ $id }}" {{ ($servicingContract->updated_by ? $servicingContract->updated_by->id : old('updated_by_id')) == $id ? 'selected' : '' }}>{{ $updated_by }}</option>
                    @endforeach
                </select>
