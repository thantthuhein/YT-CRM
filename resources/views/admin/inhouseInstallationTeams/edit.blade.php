...
<div class="card content-card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.inhouseInstallationTeam.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.inhouse-installation-teams.update", [$inhouseInstallationTeam->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('servicing_team') ? 'is-invalid' : '' }}" name="servicing_team_id" id="servicing_team_id">
                    @foreach($servicing_teams as $id => $servicing_team)
                        <option value="{{ $id }}" {{ ($inhouseInstallationTeam->servicing_team ? $inhouseInstallationTeam->servicing_team->id : old('servicing_team_id')) == $id ? 'selected' : '' }}>{{ $servicing_team }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('inhouse_installation') ? 'is-invalid' : '' }}" name="inhouse_installation_id" id="inhouse_installation_id">
                    @foreach($inhouse_installations as $id => $inhouse_installation)
                        <option value="{{ $id }}" {{ ($inhouseInstallationTeam->inhouse_installation ? $inhouseInstallationTeam->inhouse_installation->id : old('inhouse_installation_id')) == $id ? 'selected' : '' }}>{{ $inhouse_installation }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('created_by') ? 'is-invalid' : '' }}" name="created_by_id" id="created_by_id">
                    @foreach($created_bies as $id => $created_by)
                        <option value="{{ $id }}" {{ ($inhouseInstallationTeam->created_by ? $inhouseInstallationTeam->created_by->id : old('created_by_id')) == $id ? 'selected' : '' }}>{{ $created_by }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('updated_by') ? 'is-invalid' : '' }}" name="updated_by_id" id="updated_by_id">
                    @foreach($updated_bies as $id => $updated_by)
                        <option value="{{ $id }}" {{ ($inhouseInstallationTeam->updated_by ? $inhouseInstallationTeam->updated_by->id : old('updated_by_id')) == $id ? 'selected' : '' }}>{{ $updated_by }}</option>
                    @endforeach
                </select>
